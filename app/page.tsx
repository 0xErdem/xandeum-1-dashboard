...
const CACHE_KEY = 'xandeum_os_master_v1';
...
function calculateHealthScore(node: any, isVote: boolean): number {
    ...
    // 3. Observer (Oy kullanmayan) Cezası
    if (!isVote) score -= 10;
    // 4. İletişim (IP) Cezası
...
export default function Home() {
    ...
    // UI Selection
    const [selectedNode, setSelectedNode] = useState<any>(null); // Simple Mode Panel
    const [advancedSelectedNode, setAdvancedSelectedNode] = useState<any>(null); // Advanced Modal
    ...
    // --- EFFECT 1: FETCH HISTORICAL DATA (SUPABASE) ---
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    setDbHistory(data);
                    // Sadece ilk yüklemede logla, sürekli spam yapma
    ...
    useEffect(() => {
            ...
            try {
                ...
                let baseNodes = gossipNodes.map(node => {
                    ...
                    if (prodData && prodData.leaderSlots > 0) {
                        ...
                        // Eğer üretim verisi yoksa ama validatörse, lag'e göre tahmin et (Görsel doluluk için)
                    ...
                    // APY (Commission based)
                    const commission = voteData ? voteData.commission : 0;
                    const baseAPY = 7.5; // Base Inflation
                    const perfFactor = healthScore / 100;
                    const commFactor = (100 - commission) / 100;
                    const apy = voteData ? (baseAPY * perfFactor * commFactor).toFixed(2) : "0.00";
                ...
                const processGeo = async () => {
                    ...
                    for (let i = 0; i < resolveLimit; i++) {
                        ...
                        if (cache[ip]) {
                            ...
                            if (i === 10) setAllNodes([...updatedNodes]);
                            continue;
                        }
                        ...
                        try {
                            ...
                            if (res.ok) {
                                ...
                                if (!geo.error) {
                                    const geoData = { lat: geo.latitude, lng: geo.longitude, city: geo.city, country: geo.country_name, isp: geo.org };
                                    cache[ip] = geoData;
        ...
        const interval = setInterval(() => {
            ...
            if (Math.random() > 0.85) {
                const events = ["Block propogated", "Gossip synced", "Ledger updated", "Handshake accepted", "Vote cast"];
    ...
    const analytics = useMemo(() => {
        ...
        const epochData = Array.from({ length: 7 }, (_, i) => ({
            day: `T-${7-i}h`,
            load: Math.min(100, Math.max(0, epochProgress + (Math.random() * 10 - 5))), 
            limit: 100
        }));
                ...
                {viewMode === 'advanced' && (
                    <div className="absolute inset-0 z-20 pt-44 px-8 pb-8 overflow-y-auto custom-scrollbar animate-in fade-in duration-500 bg-black/80 backdrop-blur-md pointer-events-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                            ...
                            {/* NETWORK GROWTH (REAL HISTORICAL DATA) */}
